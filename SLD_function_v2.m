...
%     Xfinal = 3.5e-2; % in [m];
    ...
    Zinitial = pars.z_ini; % in [m]
    Zfinal   = pars.z_end; % in [m]
    Xinitial = pars.x_ini;  % in [m];
    Xfinal = pars.x_end; % in [m];

    P = pars.P; % number of points
    BW = pars.BW; % [MHz]
    nb_lambda_axial = pars.nb_lambda_axial;
...
%     nb_lambda_lateral = nb_lambda_axial; %round(nb_lambda_axial/2);
    nb_lambda_lateral = pars.nb_lambda_lateral;
...
%                 z0d = z0 + (nz-1) - (nw-1)/2;
    ...
    for i = 1 : nb_ROI_axial
        ...
        for j = 1 : nb_ROI_lateral
            ...
            ref_block_pro = remove_DC_in_pixel_ref(squeeze(mean(ref_block_pro)), ref_block_pro, mat_window_pro);
            ref_block_dis = remove_DC_in_pixel_ref(squeeze(mean(ref_block_dis)), ref_block_dis, mat_window_dis);
            data_block_pro = remove_DC_in_pixel    (mean(data_block_pro), data_block_pro, mat_window_pro);
            data_block_dis = remove_DC_in_pixel    (mean(data_block_dis), data_block_dis, mat_window_dis);
                
            % Averaged power spectra
            tf_ref_pro = power_spectrum_averaged_in_pixel_ref(ref_block_pro,P,0,0);
            tf_ref_dis = power_spectrum_averaged_in_pixel_ref(ref_block_dis,P,0,0);
            tf_data_pro = power_spectrum_averaged_in_pixel    (data_block_pro,P,0,0);
            tf_data_dis = power_spectrum_averaged_in_pixel    (data_block_dis,P,0,0);
            ...
            if ( j == floor(2.5*nb_ROI_lateral/5) )
                if ( i == ceil(nb_ROI_axial/10))
                    ...
                    plot( Xfreq,  10*log10(tf_ref_pro/(max(tf_ref_pro))), 'k--', 'Linewidth', 1), grid minor;
                    ...
                    plot( Xfreq,  10*log10(tf_ref_dis/(max(tf_ref_dis))), 'k--', 'Linewidth', 1), grid minor;
                ...
                elseif ( i == round(nb_ROI_axial/2))
                    ...
                    plot( Xfreq,  10*log10(tf_ref_pro/(max(tf_ref_pro))), 'r--', 'Linewidth', 1), grid minor;
                                        

                    %%%% DISTAL %%%%
                    figure(172), 
                    plot( Xfreq,  10*log10(tf_data_dis/(max(tf_data_dis))), 'r', 'Linewidth', 2), hold on;
                    plot( Xfreq,  10*log10(tf_ref_dis/(max(tf_ref_dis))), 'r--', 'Linewidth', 1), grid minor;
                ...
                elseif ( i == floor(9*nb_ROI_axial/10))
                    ...
                    plot( Xfreq,  10*log10(tf_ref_pro/(max(tf_ref_pro))), 'b--', 'Linewidth', 1), grid minor;
                    ...
                    plot( Xfreq,  10*log10(tf_ref_dis/(max(tf_ref_dis))), 'b--', 'Linewidth', 1), grid minor;
            ...
            if (i==ceil(nb_ROI_axial/10) && j==floor(2.5*nb_ROI_lateral/5))
                    ...
                    figure (17),
                    subplot 211,
                    plot( Xfreq,  10*log10(tf_ref_pro/(max(tf_ref_pro))), 'k', 'Linewidth', 1), hold on, grid minor;
