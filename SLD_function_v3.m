function [SLD, DATA, REF] = SLD_function_v3(DATA, REF, pars)
% function [SLD] = SLD_function_v3(DATA, REF, pars)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                % B MODE ORIGINAL PLOT (OPTIONAL)
%                 figure; set(1,'units','normalized','outerposition',[0 0 1 1]);
%                 set(gca,'FontSize',font);
%                 imagesc(x,z,Im_db_ori); axis image; colormap gray; caxis([0-dyn_range 0]);
%                 hb1 = colorbar;
...
%                 end     
                ...
                % OPTMIZATION WAY 1
%                 att_ref_map = bsxfun(@times, ones(m, n, length(att_ref)), reshape(att_ref, 1, 1, [])); 
                % OPTMIZATION WAY 2
                att_ref_map = ones(m, n, length(att_ref)) .* reshape(att_ref, 1, 1, []);
                % OPTMIZATION WAY 3
%                 size_map = [m, n, length(att_ref)];
%                 att_ref_map = repmat(reshape(att_ref, [1, 1,
%                 length(att_ref)]), size_map); % there might be an error
%                 here
...
%                 clearvars -except case_list angle_list db_size font frame ff ss angle
%                 close all;       
%                 continue
                ...
                SLD_term = b;
%                 save ([output_dir,'\SLD_data_inc1.mat'],'SLD_term','A','Im_db','Im_db_ori','x','z',...
%                     'x_ori', 'z_ori');
